...
public class UIJobs : MonoBehaviour, IScreen
{
  ...
  void Start()
  {
    ...
    locations = jobs.Q<VisualElement>("Locations");
    locationInfoType = jobs.Q<Label>("LocationInfoType");
    locationInfoTimezone = jobs.Q<Label>("LocationInfoTimezone");
    locationInfoDifficulty = jobs.Q<Label>("LocationInfoDifficulty");
    locationInfoPollutionRate = jobs.Q<Label>("LocationInfoPollutionRate");
    locationInfoPollution = jobs.Q<Label>("LocationInfoPollution");
    locationInfoReward = jobs.Q<Label>("LocationInfoReward");
    locationInfoStart = jobs.Q<Button>("LocationInfoStart");
    locationInfoStart.clicked += () => game.StartJob(activeJobIndex);
    locationInfo = jobs.Q<VisualElement>("LocationInfo");
    refreshTimer = jobs.Q<Label>("RefreshTimer");
  ...
  {
    refreshTimer.text = "Time Until Zone Refresh: " + Mathf.FloorToInt(game.refreshTimer);

    if (game.refreshTimer <= 0.5f)
    {
      HideLocationInfo();
    }
  }

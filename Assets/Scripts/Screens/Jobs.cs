...
public class Jobs : MonoBehaviour, IScreen
{
    ...
    List<int> activeLocations = new List<int>();
    ...
    {
        while (activeLocations.Count < 5)
        {
            var index = Mathf.FloorToInt(Random.Range(0, locations.Length));

            if (!activeLocations.Contains(index))
            {
